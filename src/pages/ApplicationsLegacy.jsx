// Legacy applications page (kept for reference).
...
const Applications = () => {
  ...
  // ─── Data fetching ──────────────────────────────────────────────────────────
  const fetchApplications = useCallback(async (page = 1, pageSize = 10, currentFilters = { status: '', search: '' }) => {
    setLoading(true);
    try {
  ...
  useEffect(() => {
    fetchApplications(1, 10, { status: '', search: '' });
    fetchStats();
